# CMPE-255-HW2

# CMPE 255 — Data Mining

## 1. Introduction to Python

**Colab:** [Open My Google Colab](https://colab.research.google.com/drive/10Dj_Bjl6dufeY0zXUA7ozZ6L1pXKEfJj)

**Executed Notebook:** [View Executed Notebook](./01-Intro-to-Python/Intro_to_Python.ipynb)

**Video:** [https://youtu.be/BHGSsZtVxaA]

---

## 2. Introduction to NumPy

**Colab:** [Open My Google Colab](https://colab.research.google.com/drive/1DM_MCjCC6IzY2WK0vHlk90Ri7OAsT6df)

**Executed Notebook:** [View Executed Notebook](./02-Intro-to-numpy/Intro_to_numpy.ipynb)

**Video:** [https://youtu.be/kHpP4DrfbHY]

---

## 3. Introduction to Pandas

**Colab:** [Open My Google Colab](https://colab.research.google.com/drive/1nbhIkp1eRIyWhmJ09xZeDLtGB5yZgRdT)

**Executed Notebook:** [View Executed Notebook](./03-Intro-to-pandas/Copy_of_Intro_to_pandas.ipynb)

**Video:** [https://youtu.be/DzoopyImN7s]

---

## 4. Introduction to Matplotlib

**Colab:** [Open My Google Colab](YOUR_COPIED_MATPLOTLIB_COLAB_LINK)

**Executed Notebook:** [View Executed Notebook](./04-Intro-to-Matplotlib/Copy_of_Intro_to_Matplotlib.ipynb)

**Video:** [https://youtu.be/YUDzhxXDP4s]

---

## 5. Linear Algebra #1

**Colab:** [Open My Google Colab](YOUR_COPIED_LINEAR_ALGEBRA_1_COLAB_LINK)

**Executed Notebook:** [View Executed Notebook](./06-Intro-to-linear-algebra-1/Copy_of_intro_to_linear_algebra_1.ipynb)

**Video:** [YouTube Video]

---

## 6. Linear Algebra #2

**Colab:** [Open My Google Colab](YOUR_COPIED_LINEAR_ALGEBRA_2_COLAB_LINK)

**Executed Notebook:** [View Executed Notebook](./07-Intro-to-linear-algebra-2/Copy_of_intro_to_linear_algebra_2.ipynb)

**Video:** [YouTube Video]

---

## 7. Linear Algebra #3

**Colab:** [Open My Google Colab](YOUR_COPIED_LINEAR_ALGEBRA_3_COLAB_LINK)

**Executed Notebook:** [View Executed Notebook](./08-Intro-to-linear-algebra-3/Copy_of_intro_to_linear_algebra_3.ipynb)

**Video:** [YouTube Video]

---

## 8. Introduction to Probability

**Colab:** [Open My Google Colab](https://colab.research.google.com/drive/1Ea68D7zb9Mu7oZig288EVq1aqzbUc3U6)

**Executed Notebook:** [View Executed Notebook](./05-Intro-to-probability/Copy_of_intro_to_probability.ipynb)

**Video:** [YouTube Video]

---

## 9. Introduction to Statistics

**Source:** [Google Drive / Statistics Notebook](https://drive.google.com/file/d/1IkMQOZVW5JQUCATRW2MbZPKCBVqlD4w6/view?usp=sharing)

**Executed Notebook:** Not currently available in the GitHub folder structure.

**Video:** [YouTube Video]

---

## 10. Probability and Statistics for Data Science #1

**Colab:** [Open My Google Colab](https://colab.research.google.com/drive/1Ulo_oAalDK1diBEyy7F1RwsmqpXnnEds)

**Executed Notebook:** [View Executed Notebook](./13-Probability-Fundamentals-1/Copy_of_probability_fundamentals_1.ipynb)

**Video:** [YouTube Video]

---

## 11. Probability and Statistics for Data Science #2

**Colab:** [Open My Google Colab](https://colab.research.google.com/drive/10V5Mf9b3ZowR_-e78-4vKwUptPXfSzIP#scrollTo=gaVljjz0Vegm)

**Executed Notebook:** [View Executed Notebook](./14-Probability-Fundamentals-2/Copy_of_probability_fundamentals_2.ipynb)

**Video:** [YouTube Video]

---

## 12. Probability and Statistics for Data Science #3

**Colab:** [Open My Google Colab](https://colab.research.google.com/drive/1bO_ax8q_wTJ5HrZksk_EcC-1vE1fUPbT)

**Executed Notebook:** [View Executed Notebook](./15-Probability-Fundamentals-3/Copy_of_probability_fundamentals_3.ipynb)

**Video:** [YouTube Video]

---

## 13. Intro to Calculus #1

**Colab:** [Open My Google Colab](https://colab.research.google.com/drive/1euBXLqidelKsk8FhBZF726JELTmEGNxG)

**Executed Notebook:** [View Executed Notebook](./10-Intro-to-Calculus-1/Copy_of_intro_to_calc_1.ipynb)

**Video:** [YouTube Video]

---

## 14. Intro to Calculus #2

**Colab:** [Open My Google Colab](https://colab.research.google.com/drive/1VF22nCsH6tocZ1uKY1KGU4i6IpQKvyBb#scrollTo=eZFX7UkBXX-I)

**Executed Notebook:** [View Executed Notebook](./11-Intro-to-Calculus-2/Copy_of_intro_to_calc_2.ipynb)

**Video:** [YouTube Video]

---

## 15. Intro to Calculus #3

**Colab:** [Open My Google Colab](https://colab.research.google.com/drive/1PAfNtX37saX6wvoM4t8UmBBJPQ56xvW9#scrollTo=6jezUzGpyL55)

**Executed Notebook:** [View Executed Notebook](./12-Intro-to-Calculus-3/Copy_of_intro_to_calculus_3.ipynb)

**Video:** [YouTube Video]

---

## 16. Foundations of Learning

**Colab:** [Open My Google Colab](https://colab.research.google.com/drive/1tuZ7NAx6VC2qpyIG3RM4nJXmzN4si0VS)

**Executed Notebook:** [View Executed Notebook](./16-Intro-to-Foundations/Copy_of_intro_to_foundations.ipynb)

**Video:** [YouTube Video]

---

# GitHub Repository Structure

The executed notebooks are organized into folders in the GitHub repository.

```text
CMPE-255-HW2/
│
├── README.md
│
├── 01-Intro-to-Python/
│   └── Intro_to_Python.ipynb
│
├── 02-Intro-to-numpy/
│   └── Intro_to_numpy.ipynb
│
├── 03-Intro-to-pandas/
│   └── Copy_of_Intro_to_pandas.ipynb
│
├── 04-Intro-to-Matplotlib/
│   └── Copy_of_Intro_to_Matplotlib.ipynb
│
├── 05-Intro-to-probability/
│   └── Copy_of_intro_to_probability.ipynb
│
├── 06-Intro-to-linear-algebra-1/
│   └── Copy_of_intro_to_linear_algebra_1.ipynb
│
├── 07-Intro-to-linear-algebra-2/
│   └── Copy_of_intro_to_linear_algebra_2.ipynb
│
├── 08-Intro-to-linear-algebra-3/
│   └── Copy_of_intro_to_linear_algebra_3.ipynb
│
├── 10-Intro-to-Calculus-1/
│   └── Copy_of_intro_to_calc_1.ipynb
│
├── 11-Intro-to-Calculus-2/
│   └── Copy_of_intro_to_calc_2.ipynb
│
├── 12-Intro-to-Calculus-3/
│   └── Copy_of_intro_to_calculus_3.ipynb
│
├── 13-Probability-Fundamentals-1/
│   └── Copy_of_probability_fundamentals_1.ipynb
│
├── 14-Probability-Fundamentals-2/
│   └── Copy_of_probability_fundamentals_2.ipynb
│
├── 15-Probability-Fundamentals-3/
│   └── Copy_of_probability_fundamentals_3.ipynb
│
└── 16-Intro-to-Foundations/
    └── Copy_of_intro_to_foundations.ipynb
